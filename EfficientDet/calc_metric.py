...
if __name__ == '__main__':
    ...
    box_file = "data/gs_data_final/multiview_measurement.json"
    gt_file = "data/gs_data_final/multiview_measurement_gt.json"
    ...
    for pp in range(2):
        for p in tqdm(place.keys(), desc=f'Calc Metric for : ', leave=False):
            ...
            View = MultiViewCoordinate(**place[p])
    ...
    # person results
    single_v0_person_recall = np.sum(results[0]["is_detected_v0"]) / np.sum(results[0]["is_available_v0"])
    single_v1_person_recall = np.sum(results[0]["is_detected_v1"]) / np.sum(results[0]["is_available_v1"])

    multi_single_v0_person_recall = np.sum(results[0]["is_detected_v0"]) / len(results[0]["is_available_v1"])
    ...
    multi_person_distance_error = np.sum(np.array(results[0]["multi_distance_error"]) * detected) / np.count_nonzero(detected) # * multi_person_recall + 1.285 * (1-multi_person_recall)
    multi_person_az_error = np.sum(np.array(results[0]["multi_az_error"]) * detected) / np.count_nonzero(detected) # * multi_person_recall + 7.9 * (1-multi_person_recall)
    multi_person_el_error = np.sum(np.array(results[0]["multi_el_error"]) * detected) / np.count_nonzero(detected) # * multi_person_recall + 8.23 * (1-multi_person_recall)
    ...
    # phone results
    single_v0_phone_recall = np.sum(results[1]["is_detected_v0"]) / np.sum(results[1]["is_available_v0"])
    single_v1_phone_recall = np.sum(results[1]["is_detected_v1"]) / np.sum(results[1]["is_available_v1"])

    multi_single_v0_phone_recall = np.sum(results[1]["is_detected_v0"]) / len(results[1]["is_available_v1"])
    ...
    multi_phone_distance_error = np.sum(np.array(results[1]["multi_distance_error"]) * detected) / np.count_nonzero(detected) # * multi_phone_recall + 1.285 * (1-multi_phone_recall)
    multi_phone_az_error = np.sum(np.array(results[1]["multi_az_error"]) * detected) / np.count_nonzero(detected) # * multi_phone_recall + 7.9 * (1-multi_phone_recall)
    multi_phone_el_error = np.sum(np.array(results[1]["multi_el_error"]) * detected) / np.count_nonzero(detected) # * multi_phone_recall + 8.23 * (1-multi_phone_recall)
...
result_table = [["Multi-view SVWC", f"{multi_person_precision*100:.1f}/{multi_person_recall*100:.1f}", f"{multi_phone_precision*100:.1f}/{multi_phone_recall*100:.1f}", f"{multi_phone_distance_error*100:.2f}", f"{multi_phone_az_error:.2f}/{multi_phone_el_error:.2f}"],
                ["Single-view SVWC", f"{single_person_precision*100:.1f}/{single_person_recall*100:.1f}", f"{single_phone_precision*100:.1f}/{single_phone_recall*100:.1f}", f"{single_phone_distance_error*100:.2f}", f"{(single_v0_phone_az_error+single_v1_phone_az_error)/2:.2f}/{(single_v0_phone_el_error+single_v1_phone_el_error)/2:.2f}"],
                ["Conventional 5G NR", f"-", f"-", f"128.5", f"7.9/8.23"]]

df = pd.DataFrame(result_table, columns = ["Technique", "Human precision/recall (%)", "Cell phone precision/recall (%)", "distance error (cm)", "azimuth/elevation angle error (degree)"])
